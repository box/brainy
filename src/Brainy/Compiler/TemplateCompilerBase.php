    ...
    public function compileTag($tag, $args, $parameter = array(), $param2 = null, $param3 = null) {
        ...
        if (strlen($tag) < 6 || substr($tag, -5) !== 'close') {
            ...
            if (isset($this->smarty->registered_plugins[Brainy::PLUGIN_FUNCTION][$tag])) {
                return $this->callTagCompiler('private_registered_' . Brainy::PLUGIN_FUNCTION, $args, $parameter, $tag);
            }
            if (isset($this->smarty->registered_plugins[Brainy::PLUGIN_BLOCK][$tag])) {
                return $this->callTagCompiler('private_registered_' . Brainy::PLUGIN_BLOCK, $args, $parameter, $tag);
            }
        ...
        if (isset($this->smarty->registered_plugins[Brainy::PLUGIN_BLOCK][$base_tag]) || isset($this->default_handler_plugins[Brainy::PLUGIN_BLOCK][$base_tag])) {
            return $this->callTagCompiler('private_registered_block', $args, $parameter, $tag);
        }
        // block plugin?
        if ($function = $this->getPlugin($base_tag, Brainy::PLUGIN_BLOCK)) {
